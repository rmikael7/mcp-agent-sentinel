 ...
 */
export async function getLiveNewsItems(forceRefresh = false) {
    const now = Date.now();
    ...
    const sortedItems = Array.from(uniqueItemsMap.values()).sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime());
