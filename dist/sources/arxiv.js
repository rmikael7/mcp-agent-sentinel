...
function parseArxivAtom(xml) {
    ...
    for (let i = 1; i < entries.length; i++) {
        ...
        if (lowerTitle.includes('agent') || lowerSummary.includes('agent'))
            tags.push('agents');
        if (lowerTitle.includes('llm') || lowerSummary.includes('language model'))
            tags.push('llm');
        if (lowerTitle.includes('reasoning') || lowerSummary.includes('reasoning'))
            tags.push('reasoning');
        if (lowerTitle.includes('benchmark') || lowerSummary.includes('eval'))
            tags.push('benchmarks');
        if (lowerTitle.includes('rag') || lowerSummary.includes('retrieval'))
            tags.push('rag');
        const isHighImpact = tags.includes('agents') || tags.includes('llm') || tags.includes('reasoning');
        const personas = ['dev', 'product', 'creator'];
        if (tags.includes('benchmarks') || tags.includes('agents'))
            personas.push('investor');
        items.push({
            id: `arxiv-${paperId}`,
